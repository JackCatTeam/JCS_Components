...
#sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" JCS_Components.podspec
...
# 验证podspec文件是否合法
pod lib lint JCS_Components.podspec --sources=${gitsource}  --verbose --allow-warnings --use-libraries

LineNumber=`grep -nE 's.version.*=' JCS_Components.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/^\([ ]*s.version[ ]*=[ ]*\"\)\(.*\)\(\"[ ]*\)$/\1${tag}\3/g" JCS_Components.podspec
